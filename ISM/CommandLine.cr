module ISM
    ...
    class CommandLine
        ...
        def runAsSuperUser(validCondition = true, &)
            if validCondition
                uidResult = LibC.setresuid( realId: 0,
                                            effectiveId: 0,
                                            savedId: ISM::Default::CommandLine::Id)
        ...
        def showUnconfiguredSystemComponentMessage(component : ISM::SoftwareInformation)
            puts
            puts "#{ISM::Default::CommandLine::UnconfiguredSystemComponentText1.colorize(:yellow)}#{component.fullName.colorize(:green)}#{ISM::Default::CommandLine::UnconfiguredSystemComponentText2.colorize(:yellow)}"
        end
        ...
        def buildTasksFile
            ...
            Process.run("CRYSTAL_WORKERS=#{Ism.settings.systemMakeOptions[2..-1]} crystal build #{ISM::Default::Filename::Task}.cr -o #{@settings.rootPath}#{ISM::Default::Path::RuntimeDataDirectory}#{ISM::Default::Filename::Task} -f json",
                        error: processResult,
                        shell: true,
