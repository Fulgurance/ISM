  ...
  class CommandLine
    ...
    def softwareIsRequestedSoftware(software : ISM::SoftwareInformation, requestedSoftwareVersionNames = Array(String).new) : Bool
      if requestedSoftwareVersionNames.empty?
        @requestedSoftwares.any? { |entry| entry.fullVersionName == software.fullVersionName }
      else
        requestedSoftwareVersionNames.any? { |entry| entry == software.fullVersionName }
      end
    end
    ...
    def softwareIsInstalled(software : ISM::SoftwareInformation) : Bool
      if File.exists?(software.installedFilePath)
        ...
        (equalScore == software.options.size)
      else
        false
      end
    end
    ...
    def softwaresAreCodependent(software1 : ISM::SoftwareInformation, software2 : ISM::SoftwareInformation) : Bool
      software1.allowCodependencies.includes?(software2.fullName) && software2.allowCodependencies.includes?(software1.fullName) && !software1.passEnabled && !software2.passEnabled
    end
    ...
    def getNeededSoftwares : Array(ISM::SoftwareInformation)
      ...
      getSortedDependencies(dependencyTable)
    end
