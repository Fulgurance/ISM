module ISM
    ...
    class Software
        ...
        def prepareChrootDev
            requestedCommands = "/usr/bin/sudo /usr/bin/mount -v --bind /dev #{Ism.settings.rootPath}/dev"
        ...
        def prepareChrootDevPts
            requestedCommands = "/usr/bin/sudo /usr/bin/mount -v -t devpts devpts -o gid=5,mode=0620 #{Ism.settings.rootPath}/dev/pts"
        ...
        def prepareChrootProc
            requestedCommands = "/usr/bin/sudo /usr/bin/mount -v -t proc proc #{Ism.settings.rootPath}/proc"
        ...
        def prepareChrootSysfs
            requestedCommands = "/usr/bin/sudo /usr/bin/mount -v -t sysfs sysfs #{Ism.settings.rootPath}/sys"
        ...
        def prepareChrootTmpfs
            requestedCommands = "/usr/bin/sudo /usr/bin/mount -v -t tmpfs tmpfs #{Ism.settings.rootPath}/run"
        ...
        def prepareChrootNetworkConfiguration
            requestedCommands = "/usr/bin/sudo /usr/bin/cp /etc/resolv.conf #{Ism.settings.rootPath}/etc/resolv.conf"
        ...
        def workDirectoryPath : String
            return Ism.systemInformation.handleUserAccess ? "/#{Path::SourcesDirectory}"+@information.port+"/"+@information.name+"/"+@information.version : Ism.settings.sourcesPath+@information.port+"/"+@information.name+"/"+@information.version
        ...
        def builtSoftwareDirectoryPath : String
            return Ism.systemInformation.handleUserAccess ? "/#{@information.builtSoftwareDirectoryPath}" : "#{Ism.settings.rootPath}#{@information.builtSoftwareDirectoryPath}"
        ...
        def directoryContent(path : String, matchHidden = false) : Array(String)
            ...
            fileList = Dir.glob((Ism.systemInformation.handleUserAccess ? Ism.settings.rootPath+path : path), match: (matchHidden ? File::MatchOptions.glob_default : File::MatchOptions::None))

            return fileList.map { |file| (Ism.systemInformation.handleUserAccess ? file[(Ism.settings.rootPath.size-1)..-1] : file)}
        ...
        def checkFile(archive : String, sha512 : String)
            ...
            if archiveSha512 != sha512
                puts "The checksum do not match with the downloaded file. The file can be corrupted or compromised."
            end
        ...
        def makeLink(target : String, path : String, type : Symbol)
            ...
            when :symbolicLinkByOverwrite
                symlinkRealPath = "#{Ism.settings.rootPath}/#{path}"
        ...
        def runUserAddCommand(arguments : String)
            requestedCommands = "sudo useradd -R #{Ism.settings.rootPath} #{arguments}"
        ...
        def runUserDelCommand(arguments : String)
            requestedCommands = "sudo userdel -R #{Ism.settings.rootPath} #{arguments}"
        ...
        def runGroupAddCommand(arguments : String)
            requestedCommands = "sudo groupadd -R #{Ism.settings.rootPath} #{arguments}"
        ...
        def runGroupDelCommand(arguments : String)
            requestedCommands = "sudo groupdel -R #{Ism.settings.rootPath} #{arguments}"

            process = Ism.runSystemCommand(requestedCommands, shell: true)
            ...
            loop do
                ...
                result.each do |line|
                    ...
                    if text.starts_with?(Default::KconfigKeywords[:source]) && !text.includes?("Kconfig.include")
                        ...
                        mainArchitecture = (Ism.systemInformation.handleUserAccess ? Ism.settings.chrootArchitecture : Ism.settings.systemArchitecture).gsub(/_.*/,"")
