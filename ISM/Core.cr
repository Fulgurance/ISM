module ISM
    ...
    module Core
        ...
        def self.runTasks(tasks, quiet = false, asRoot = false, viaChroot = false, input = Process::Redirect::Inherit, output = Process::Redirect::Inherit, error = Process::Redirect::Inherit) : Process::Status
            ...
            viaChrootCommand = "HOME=/var/lib/ism sudo chroot #{asRoot ? "" : "--userspec=#{Security::Default::SystemUserId}:#{Security::Default::SystemUserId}"} #{commandLineSettings.rootPath}"
        ...
        def self.runSystemCommand(command : String, path = commandLineSettings.installByChroot ? "/" : commandLineSettings.rootPath, environment = Hash(String, String).new, environmentFilePath = String.new, quiet = false, asRoot = false, viaChroot = true, input = Process::Redirect::Inherit, output = Process::Redirect::Inherit, error = Process::Redirect::Inherit) : Process::Status
            ...
            targetedSystemHandleUserAccess = ISM::CommandLineSystemInformation.loadConfiguration(ISM::CommandLineSystemInformation.filePath).handleUserAccess

            superuser = (asRoot && systemHandleUserAccess)
            chroot = (commandLineSettings.installByChroot && viaChroot)

            environmentCommand = String.new
            systemEnvironment = String.new
            ...
            if chroot
                ...
                set +h
                LC_ALL=POSIX
                PATH=#{!targetedSystemHandleUserAccess ? "#{commandLineSettings.toolsPath}/bin:" : ""}/usr/bin:/usr/sbin
                ENVIRONMENT
            end
            ...
            #TRACELOG-------------------------------------------------------------
            ISM::Core::TraceLog.record( accessor:   "Core",
                                        function:   "runSystemCommand",
                                        message:    <<-TEXT
                                        Running command (asRoot: #{superuser}, viaChroot: #{chroot}):
                                        #{command}
                                        TEXT
            )
            #-------------------------------------------------------------TRACELOG
