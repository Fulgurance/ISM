        ...
        class VersionSwitch < ISM::CommandLineOption
            ...
            def start
                if ARGV.size == 2
                    ...
                    if !process.success?
                        Process.run("git switch #{currentVersion}",
                                    shell: true,
                                    chdir: "/"+ISM::Default::Path::LibraryDirectory)
                    end

                    Process.run("CRYSTAL_WORKERS=#{Ism.settings.systemMakeOptions[2..-1]} crystal build --release Main.cr -o #{Ism.settings.rootPath+ISM::Default::Path::BinaryDirectory+ISM::Default::Filename::IsmBinary}",
                                shell: true,
                                chdir: "/"+ISM::Default::Path::LibraryDirectory)

                    Process.run("git update-ref -d /refs/heads/#{previousVersion}",
                                shell: true,
                                chdir: "/"+ISM::Default::Path::LibraryDirectory)
                end
            end
