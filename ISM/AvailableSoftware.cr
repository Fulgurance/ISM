module ISM
  class AvailableSoftware
    ...
    private def intervalComparator(request : String) : Bool
      ...
      (brackets && separator && comparators)
    end
    ...
    def getVersionByCondition(condition : String, returnMaximum = true) : ISM::SoftwareInformation
      ...
      temp.empty? ? ISM::SoftwareInformation.new : (returnMaximum ? temp.max_by { |entry| SemanticVersion.parse(entry.version) } : temp.min_by { |entry| SemanticVersion.parse(entry.version) })
    end
