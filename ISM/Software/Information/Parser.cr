    ...
    class Software
        ...
        class Information
            ...
            def toInformationFile(path : String)
                ...
                SectionKeywords.values.each do |section|

                    case section
                    when SectionKeywords[:port]
                        appendLineToFile(line: section, path: path)
                        appendLineToFile(line: "    #{@port}", path: path)
                    when SectionKeywords[:name]
                        appendLineToFile(line: section, path: path)
                        appendLineToFile(line: "    #{@name}", path: path)
                    when SectionKeywords[:version]
                        appendLineToFile(line: section, path: path)
                        appendLineToFile(line: "    #{@version}", path: path)
                    when SectionKeywords[:architecture]
                        appendLineToFile(line: section, path: path)

                        @architectures.each do |architecture|
                            appendLineToFile(line: "    #{architecture}", path: path)
                        end
                    when SectionKeywords[:description]
                        appendLineToFile(line: section, path: path)
                        appendLineToFile(line: "    #{@description}", path: path)
                    when SectionKeywords[:website]
                        appendLineToFile(line: section, path: path)
                        appendLineToFile(line: "    #{website}", path: path)
                    when SectionKeywords[:dependencies]
                        appendLineToFile(line: section, path: path)

                        @dependencies.each do |dependency|
                            appendLineToFile(line: "        @#{dependency.port}:#{dependency.name}#{dependency.version}#{dependency.options.empty? ? "" : "(#{dependency.options.join(",")})"}", path: path)
                        end
                    when SectionKeywords[:kernelDependencies]
                        appendLineToFile(line: section, path: path)

                        @kernelDependencies.each do |kernelDependency|
                            appendLineToFile(line: "    #{kernelDependency}", path: path)
                        end
                    when SectionKeywords[:options]
                        appendLineToFile(line: section, path: path)

                        @options.each do |option|
                            appendLineToFile(line: "    #{OptionKeywords[:name]}: #{option.name}", path: path)
                            appendLineToFile(line: "    #{OptionKeywords[:description]}: #{option.description}", path: path)
                            appendLineToFile(line: "    #{OptionKeywords[:active]}: #{option.active ? BoleanKeywords[:true] : BoleanKeywords[:false]}", path: path)

                            appendLineToFile(line: "    #{OptionKeywords[:dependencies]}:", path: path)
                            option.dependencies.each do |dependency|
                                appendLineToFile(line: "        @#{dependency.port}:#{dependency.name}#{dependency.version}#{dependency.options.empty? ? "" : "(#{dependency.options.join(",")})"}", path: path)
                            end

                            appendLineToFile(line: "    #{OptionKeywords[:kernelDependencies]}:", path: path)
                            option.kernelDependencies.each do |kernelDependency|
                                appendLineToFile(line: "        #{kernelDependency}", path: path)
                            end

                            appendLineToFile(line: "", path: path)
                        end
                    when SectionKeywords[:uniqueDependencies]
                        appendLineToFile(line: section, path: path)

                        @uniqueDependencies.each do |uniqueSet|
                            appendLineToFile(line: "    #{uniqueSet.join(",")}", path: path)
                        end
                    when SectionKeywords[:uniqueOptions]
                        appendLineToFile(line: section, path: path)

                        @uniqueOptions.each do |uniqueSet|
                            appendLineToFile(line: "    #{uniqueSet.join(",")}", path: path)
                        end
                    when SectionKeywords[:allowCodependencies]
                        appendLineToFile(line: section, path: path)

                        @allowCodependencies.each do |codependency|
                            appendLineToFile(line: "    #{codependency}", path: path)
                        end
                    else
